...
- **New name**: Renamed extension to **Datly — Safe Dart Data Classes** (display name). Internal package id, settings keys, and command identifiers are unchanged so existing installs and `settings.json` keep working.
- **New icon**: Faceted geometric "D" mark on a deep-blue gradient, replacing the prior document/yellow design.

### Package & Documentation

- **package.json cleanup**: Sharpened description, replaced duplicate keyword variations with 12 discoverable terms (`flutter`, `json to dart`, `copyWith`, `sealed`, `freezed alternative`, etc.), added `license`, `bugs`, `homepage`, `galleryBanner`, removed the redundant `author` block, and dropped noisy `"enum": [true, false]` from boolean settings.
- **README/DEPLOYMENT/TODO**: Updated all surface-level branding to "Datly", refreshed the nested-object example to show the typed `Map<dynamic, dynamic>` cast.
- **Command titles**: `Datly: Generate from class properties` / `Datly: Generate from JSON`.
- **Default header line**: Generated files now start with `// Generated by Datly` (override via `custom.headerLines`).
